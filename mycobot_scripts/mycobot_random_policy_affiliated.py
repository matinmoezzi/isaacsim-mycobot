...
class MyCobot(Robot):
    def __init__(
        ...
        if self._usd_path is None:
            '''Model's USD Path needs to be absolute!!! '''
            # self._usd_path = "../myCobot_model/mycobot_with_instance.usd" 
            self._usd_path = "/home/chay/OmniIsaacGymEnvs/omniisaacgymenvs/myCobot_model/mycobot_with_instance_v2.usd"
            # self._usd_path = "/home/chay/Desktop/MyCobot_v2/mycobot_with_instance.usd"
            # self._usd_path = "/home/chay/Downloads/mycobotv1/mycobot_with_instance_v2.usd"
...
class myCobotTask(BaseTask):
    def __init__(
        self,
        name,
        sim_config, # from Sim_Config
        env, # from VecEnvBase
        offset=None
    ) -> None:
        ...
        self._num_envs = 4 # self._task_cfg["env"]['numEnvs'] # 512
        self._env_spacing = 30.0 # self._task_cfg["env"]["envSpacing"]
        self._cartpole_positions = torch.tensor([0.0, 0.0, 0.0])

        self._reset_dist = 3.0 # self._task_cfg["env"]["resetDist"]
        self._max_push_effort = 400 # self._task_cfg["env"]["maxEffort"]
        self._max_episode_length = 500

        self._num_observations = 4
        self._num_actions = 1

        # inherited from RLTask __init__ - start
        super().__init__(name=name, offset=offset)
        self.test = False # self._cfg["test"]
        self._device = self._cfg["sim_device"] #'cuda:0'
        ...
        self.cleanup()
        # inherited from RLTask __init__ - end
        return
    ...
    def set_up_scene(self, scene) -> None:
        ...
        # inherited from RLTask set_up_scene - start
        ...
        self.set_initial_camera_params(camera_position=[5, 30, 30], camera_target=[0, 0, 0])
        if self._sim_config.task_config["sim"].get("add_distant_light", True):
            create_distant_light()
        # inherited from RLTask set_up_scene - end

        self._cartpoles = ArticulationView(prim_paths_expr="/World/envs/.*/mycobot", name="mycobot_view")
        scene.add(self._cartpoles)
        return

    def get_mycobot(self):
        cartpole = MyCobot(prim_path=self.default_zero_env_path + "/mycobot", name="mycobot", translation=self._cartpole_positions)
